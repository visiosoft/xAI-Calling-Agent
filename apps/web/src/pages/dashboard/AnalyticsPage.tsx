...
export function AnalyticsPage() {
  ...
  const kpis = [
    { label: "Total Calls", value: analytics?.totalCalls?.toLocaleString() ?? "0", icon: Phone },
    { label: "Answer Rate", value: `${(analytics?.answerRate ?? 0).toFixed(1)}%`, icon: PhoneIncoming },
    { label: "Avg Duration", value: formatDuration(analytics?.avgDuration ?? 0), icon: Clock },
    { label: "Conversion Rate", value: `${(analytics?.conversionRate ?? 0).toFixed(1)}%`, icon: TrendingUp },
  ];
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        {outcomeEntries.length > 0 ? (
          <div className="space-y-3">
            {outcomeEntries.sort(([, a], [, b]) => b - a).map(([outcome, count]) => {
              const percentage = totalOutcomes > 0 ? (count / totalOutcomes) * 100 : 0;
              const barColor = OUTCOME_COLORS[outcome] ?? "bg-primary";
              return (
                <div key={outcome}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm font-medium text-foreground">{formatLabel(outcome)}</span>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-right py-3 px-2 font-medium text-muted-foreground">Calls Made</th>
                  <th className="text-right py-3 px-2 font-medium text-muted-foreground">Answer Rate</th>
                  <th className="text-right py-3 px-2 font-medium text-muted-foreground">Conversion Rate</th>
                </tr>
              </thead>
              <tbody>
                {analytics.campaignPerformance.map((campaign) => (
                  <tr key={campaign.id} className="border-b border-border last:border-0">
                    <td className="py-3 px-2 text-foreground font-medium">{campaign.name}</td>
                    <td className="py-3 px-2 text-right text-foreground">{campaign.callsMade.toLocaleString()}</td>
                    <td className="py-3 px-2 text-right text-foreground">{campaign.answerRate.toFixed(1)}%</td>
                    <td className="py-3 px-2 text-right text-foreground">{campaign.conversionRate.toFixed(1)}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
