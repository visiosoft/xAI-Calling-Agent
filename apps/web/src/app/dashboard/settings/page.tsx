...
export default function SettingsPage() {
  ...
  useEffect(() => {
    if (settings) {
      ...
      setTelephonyProvider(settings.telephonyProvider ?? "TWILIO" as const);
  ...
  const updateTelephony = useMutation({
    mutationFn: (data: Record<string, unknown>) =>
      api.post("/api/settings/telephony", data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["settings"] });
      toast.success("Telephony settings saved successfully");
    },
    onError: () => {
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end">
            <button
              onClick={() => updateSettings.mutate({ name: orgName })}
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1.5">
              Your xAI API key for Grok voice model access
            </p>
          </div>
          <div className="flex justify-end">
            <button
              onClick={() => updateSettings.mutate({ xaiApiKey })}
              disabled={updateSettings.isPending}
              className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg font-medium hover:opacity-90 disabled:opacity-50"
            >
              <Save className="h-4 w-4" />
      ...
      <div className="rounded-xl border border-border bg-card p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {Object.values(TELEPHONY_PROVIDERS).map((provider) => (
              <button
                key={provider}
                type="button"
                onClick={() => setTelephonyProvider(provider)}
                className={cn(
              ...
              >
                <p className="font-medium text-foreground">
                  {provider === "TWILIO" as const ? "Twilio" : "Telnyx"}
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {provider === "TWILIO" as const
                    ? "Industry-standard cloud telephony"
                    : "Developer-friendly SIP trunking"}
                </p>
              </button>
            ))}
          </div>
          ...
          <div>
            ...
            />
            <p className="text-xs text-muted-foreground mt-1.5">
              Comma-separated list of phone numbers to use for outbound calls
            </p>
          </div>
          ...
          <div className="flex justify-end">
            <button
              ...
              disabled={updateTelephony.isPending}
              className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg font-medium hover:opacity-90 disabled:opacity-50"
            >
              <Save className="h-4 w-4" />
              Save
            ...
            <div className="flex items-center gap-4">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (val >= 1 && val <= 50) setMaxConcurrentCalls(val);
                }}
                className="w-20 px-3 py-2 border border-border rounded-lg bg-background text-foreground text-center focus:outline-none focus:ring-2 focus:ring-primary"
              />
            </div>
            <p className="text-xs text-muted-foreground mt-1.5">
              Maximum number of simultaneous outbound calls (1-50)
            </p>
          </div>
          <div className="flex justify-end">
            <button
              onClick={() => updateSettings.mutate({ maxConcurrentCalls })}
              disabled={updateSettings.isPending}
              className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground rounded-lg font-medium hover:opacity-90 disabled:opacity-50"
            >
              <Save className="h-4 w-4" />
